...
class Register extends Component {
  ...
  Register = (event) => {
    ...
    let url = "http://localhost/olshop/public/user/auth";
    ...
    .then(response => {
      ...
      if (logged) {
        this.setState({message: "Register sukses"});
        //menyimpan data token pada local storage
        localStorage.setItem("Token", response.data.token);
        //menyimpan data reg user ke local storage
        localStorage.setItem("user", JSON.stringify(response.data.user));
        //direct ke halaman data siswa
        window.location = "/olshop";
      } else {
        this.setState({message: "Register Gagal"});
      }
      $("#message").toast("show");
    })
    .catch(error => {
